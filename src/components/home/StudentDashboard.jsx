...
export default function StudentDashboard() {
  ...
  return <main className="dashboard-page"><header className="dashboard-header"><a className="dashboard-brand" href="/"><span>A</span><strong>Bijak belajar</strong></a><div className="dashboard-actions"><a href="/murid" title="Tukar profil / Switch profile"><span className={`avatar avatar-small avatar-${avatar.color}`}><img src={avatar.image} alt="" onError={(event) => { event.currentTarget.hidden = true; }} /><span>{avatar.mark}</span></span><span>{profile.nickname}</span><UserRound size={16} /></a><a className="exit-link" href="/"><LogOut size={16} /><span>Keluar / Exit</span></a></div></header><section className="dashboard-content">
    <div className="dashboard-welcome"><div><span className="portal-eyebrow"><Star size={16} fill="currentColor" /> Misi hari ini / Today’s mission</span><h1>Hai, {profile.nickname}!</h1><p>Pilih satu langkah kecil. Anda boleh belajar, main atau cuba sendiri.</p></div><div className={`dashboard-avatar avatar-${avatar.color}`}><img src={avatar.image} alt="" onError={(event) => { event.currentTarget.hidden = true; }} /><span>{avatar.mark}</span></div></div>
